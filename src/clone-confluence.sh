...
helpScreen() {
  echo "${scriptName} [OPTIONS]
    ...
    Bash utility for cloning a Confluence instance to another Server
...
stopLocalConf() {
    ...
    echo "Shutting down local Confluence instance (destination)...."
    /etc/init.d/confluence stop
    if [ $? -ne 0 ]; then
        read -p "Local Confluence instance did not shutdown properly, exiting? (Y/N)" yn
        case ${yn} in
            [Yy]* ) exit 1;;
            [Nn]* ) echo "Continuing....";;
        esac
...
stopRemoteConf() {
    ...
    read -p "Do you want to shutdown the remote Confluence instance (source)? (Y/N)" yn1
    ...
    if [ $? -ne 0 ]; then
        ...
        case ${yn1} in
            [Yy]* )
                ...
                local REMOTE_COMMAND="/etc/init.d/confluence stop"
                ${SSH_COMMAND} "${REMOTE_COMMAND}"
                if [ $? -ne 0 ]; then
                    read -p "Remote Confluence instance did not shutdown properly, exiting? (Y/N)" yn2
                    case ${yn2} in
                        [Yy]* ) exit 1;;
                        [Nn]* ) echo "Continuing....";;
                    esac
...
transferDatabase() {
    ...
    local SSH_COMMAND="ssh -i $(config_get SSH_PRIV_KEY) -o StrictHostKeyChecking=no $(config_get SSH_USER)@$(config_get DB_CONF_SRC_HOST)"
    local MYSQL_HOST="$(config_get DB_CONF_SRC_HOST)"
    local MYSQL_USER="$(config_get DB_CONF_SRC_USERNAME)"
    local MYSQL_PASSWORD="$(config_get DB_CONF_SRC_PASSWORD)"
    local MYSQL_DB="$(config_get DB_CONF_SRC_DBNAME)"
...
syncConfProgramFiles() {
    ...
    local RSYNC_SRC_CONF_FOLDER="$(config_get SSH_USER)@$(config_get SSH_SRC_HOST):$(config_get RSYNC_SRC_CONF_FOLDER)"
    ...
    if [ ${#SSH_COMMAND} -eq 0 ] ; then
       rsync ${RSYNC_OPTIONS} --exclude="logs" --exclude="temp" ${RSYNC_SRC_CONF_FOLDER} ${RSYNC_DST_CONF_FOLDER}
        ...
        if [ ${#RSYNC_REMOTE_SUDO} -eq 0 ] ; then
            rsync ${RSYNC_OPTIONS} --exclude="logs" --exclude="temp" -e "${SSH_COMMAND}" ${RSYNC_SRC_CONF_FOLDER} ${RSYNC_DST_CONF_FOLDER}
        else
            rsync ${RSYNC_OPTIONS} --exclude="logs" --exclude="temp" -e "${SSH_COMMAND}" --rsync-path="${RSYNC_REMOTE_SUDO}" ${RSYNC_SRC_CONF_FOLDER} ${RSYNC_DST_CONF_FOLDER}
        fi
    fi
...
syncConfDataFiles() {
    ...
    local RSYNC_SRC_CONF_DATA_FOLDER="$(config_get SSH_USER)@$(config_get SSH_SRC_HOST):$(config_get RSYNC_SRC_CONF_DATA_FOLDER)"
    local RSYNC_DST_CONF_DATA_FOLDER="$(config_get RSYNC_DST_CONF_DATA_FOLDER)"

    if [ ${#SSH_COMMAND} -eq 0 ] ; then
        rsync ${RSYNC_OPTIONS} --exclude="caches" --exclude="tmp" --exclude="log"  ${RSYNC_SRC_CONF_DATA_FOLDER} ${RSYNC_DST_CONF_DATA_FOLDER}
    else
        if [ ${#RSYNC_REMOTE_SUDO} -eq 0 ] ; then
            rsync ${RSYNC_OPTIONS} --exclude="caches" --exclude="tmp" --exclude="log" -e "${SSH_COMMAND}" ${RSYNC_SRC_CONF_DATA_FOLDER} ${RSYNC_DST_CONF_DATA_FOLDER}
        else
            rsync ${RSYNC_OPTIONS} --exclude="caches" --exclude="tmp" --exclude="log" -e "${SSH_COMMAND}" --rsync-path="${RSYNC_REMOTE_SUDO}" ${RSYNC_SRC_CONF_DATA_FOLDER} ${RSYNC_DST_CONF_DATA_FOLDER}
        fi
    fi

    # Recreate the excluded folders
    mkdir -p ${RSYNC_DST_CONF_DATA_FOLDER}/caches
    mkdir -p ${RSYNC_DST_CONF_DATA_FOLDER}/tmp
    mkdir -p ${RSYNC_DST_CONF_DATA_FOLDER}/log

    chown -R jira ${RSYNC_DST_CONF_DATA_FOLDER}
}

updateConfConfigFiles() {
    echo "Updating Confluence Configuration Files"
    ...
    local DB_NEW_CONNECTION_STRING="$(config_get DB_CONF_DST_CONNECTION_STRING)"
    local DB_NEW_USERNAME="$(config_get DB_CONF_DST_USERNAME)"
    local DB_NEW_PASSWORD="$(config_get DB_CONF_DST_PASSWORD)"

    # Update of server.xml
    local CONF_SRC_HOST="$(config_get CONF_SRC_HOST)"
    local CONF_DST_HOST="$(config_get CONF_DST_HOST)"
...
updateDBFileContent() {
    ...
    if [ ! -f ./lastconfbackup.tmp ]; then
        echo "lastconfbackup.tmp file not found! Aborting..."
        exit 1
    fi

    local DUMP=$(cat ./lastconfbackup.tmp)

    echo "Backup File Location : ${DUMP}"

    # Now we cleanup the file before creating the second DB
    local CONF_SRC_BASE_URL="$(config_get CONF_SRC_BASE_URL)"
    local CONF_DST_BASE_URL="$(config_get CONF_DST_BASE_URL)"
    local CONF_SRC_DBNAME="$(config_get DB_CONF_SRC_DBNAME)"
    local CONF_DST_DBNAME="$(config_get DB_CONF_DST_DBNAME)"


    #Look for the first match and quit
    local ORG_STRING=$(sed -n '/Current/{p;q;}'  ${DUMP})
    local UPT_STRING=${ORG_STRING/${CONF_SRC_DBNAME}/${CONF_DST_DBNAME}}
    ...
    if [[ "${OSTYPE}" == "linux-gnu" ]]; then
        sed -i "s#${CONF_SRC_BASE_URL//\./\\.}#${CONF_DST_BASE_URL}#g" ${DUMP}
    elif [[ "$OSTYPE" == "darwin"* ]]; then
        LC_CTYPE=C sed -i "" "s#${CONF_SRC_BASE_URL//\./\\.}#${CONF_DST_BASE_URL}#g" ${DUMP}
    else
        echo "Unsupported OS :${OSTYPE}. Exiting...."
        exit 1
    fi
    #Once the update is done we delete the tmp file holding the backup name to avoid double substitution
    rm -f ./lastconfbackup.tmp

    #And we create a new tmp file for the next step
    echo ${DUMP} > ./lastupdtconfbackup.tmp
...
restoreDatabase() {
    ...
    # We first check if the last backup was done and the file name saved in the proper tmp file
    if [ ! -f ./lastupdtconfbackup.tmp ]; then
        echo "lastupdtconfbackup.tmp file not found! Aborting..."
        exit 1
    fi

    local DUMP=$(cat ./lastupdtconfbackup.tmp)

    echo "Backup File Location : ${DUMP}"

    local SSH_COMMAND="ssh -i $(config_get SSH_PRIV_KEY) -o StrictHostKeyChecking=no $(config_get SSH_USER)@$(config_get DB_CONF_DST_HOST)"
    local MYSQL_HOST="$(config_get DB_CONF_DST_HOST)"
    local MYSQL_USER="$(config_get DB_CONF_DST_USERNAME)"
    local MYSQL_PASSWORD="$(config_get DB_CONF_DST_PASSWORD)"
    local MYSQL="$(config_get MYSQL_DST_MYSQL)"

    local REMOTE_MYSQL_COMMAND="${MYSQL} -u ${MYSQL_USER} -p${MYSQL_PASSWORD} -h ${MYSQL_HOST}"
    ${SSH_COMMAND} "${REMOTE_MYSQL_COMMAND}" < ${DUMP}

    rm -f ./lastupdtconfbackup.tmp
...
echo "----------------------------------------------------"
echo "Confluence Cloning is Done. Have a great day ahead !"
echo "----------------------------------------------------"
